...
function markOrderDelivered(idx) {
  ...
  if (!isPlatform) {
    sendToGoogleSheets({
      ...
      paymentType: isVeresiye ? "Veresiye" : (order.paymentMethod || "Nakit"),
      total: order.total,
      cardSales: (!isVeresiye && order.paymentMethod === "Kredi Kartı") ? order.total : 0,
      cashSales: (!isVeresiye && (order.paymentMethod === "Nakit" || !order.paymentMethod)) ? order.total : 0,
      transferSales: (!isVeresiye && (order.paymentMethod === "Banka" || order.paymentMethod === "Havale / IBAN")) ? order.total : 0,
      vatTotal: Number(vatTotal.toFixed(2)),
      isOfficial: isOrderOfficial,
